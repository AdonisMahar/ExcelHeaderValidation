...
internal class RowValidator
{
    ...
    public bool IsValidHeader(string toCheck, ValidatorConfiguration configuration)
    {
        bool isValid = true;
        ...
        {
            if (currentColumn < configuration.Columns.Values.Count)
            {
                var column = configuration.Columns.First(m => m.Key == currentColumn + 1).Value.Name;
                bool result = column.ToString().Equals(parts[currentColumn], StringComparison.Ordinal);
                if (!result)
                {

                    _errorInformation.Errors.Add(new ValidationError(currentColumn, column + " has not valid heading"));
                    IList<ValidationError> newErrors = _errorInformation.Errors;
